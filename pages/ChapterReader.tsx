...
const ChapterReader: React.FC = () => {
  ...
  useEffect(() => {
    if (currentNum > 0) {
      localStorage.setItem('dandadan_last_chapter', String(currentNum));
    }
  }, [currentNum]);
  ...
  return (
    <div className="bg-gray-100 dark:bg-[#121212] min-h-screen flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-3 h-14 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-[10px] text-bb-blue font-bold uppercase tracking-wider leading-none mb-0.5">Dandadan</span>
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            ...
            <button disabled={!nextChapter} onClick={() => nextChapter && handleNav(nextChapter.number)} className="flex items-center gap-1 px-2 py-1.5 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md dark:text-white transition-colors text-xs font-bold" title="Next chapter (→)"><span className="hidden sm:block">Next</span>
      ...
      <div className="flex-1 pt-16">
        <h1 className="sr-only">Read Dandadan Chapter {chapter.number}: {chapter.title}</h1>
        {chapter.pages.length > 0 && (
          <div className="max-w-4xl mx-auto px-4 py-4">
            <Breadcrumbs items={[
              { label: 'Home', path: '/' },
              { label: 'Dandadan', path: '/manga' },
          ...
          <div className="max-w-4xl mx-auto bg-white dark:bg-black shadow-2xl min-h-screen">
            {chapter.pages.map((pageUrl, idx) => (
              <img
                key={idx}
                src={pageUrl}
                alt={`Dandadan Chapter ${chapter.number} Page ${idx + 1}`}
        ...
        <div className="max-w-4xl mx-auto mt-12 px-4 pb-20">
          <div className="bg-[#1a1a1a] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/5">
              ...
              <button
                onClick={() => navigator.share?.({ title: `Dandadan Chapter ${chapter.number}`, url: window.location.href })}
